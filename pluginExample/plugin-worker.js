    ...
    const send = (level, args) => {
      try {
        ...
        parentPort.postMessage({
          ...
          message: msg,
          pluginId: workerData && workerData.pluginId
        });
      } catch (_) {}
    };
    console.log = (...args) => { try { orig.log.apply(console, args); } catch (_) {} send('info', args); };
    console.info = (...args) => { try { orig.info.apply(console, args); } catch (_) {} send('info', args); };
    console.warn = (...args) => { try { orig.warn.apply(console, args); } catch (_) {} send('warn', args); };
    console.error = (...args) => { try { orig.error.apply(console, args); } catch (_) {} send('error', args); };
    console.debug = (...args) => { try { orig.debug.apply(console, args); } catch (_) {} send('debug', args); };
  } catch (_) {}
})();
...
async function loadPlugin(pluginPath) {
  try {
    const pluginDir = path.dirname(
      workerData && workerData.pluginPath ? workerData.pluginPath : ''
    );
    const pluginId = workerData && workerData.pluginId ? workerData.pluginId : 'unknown';
    ...
    windowRef.path = path;
    try { windowRef.setTimeout = typeof setTimeout !== 'undefined' ? setTimeout.bind(global) : windowRef.setTimeout; } catch (_) {}
    try { windowRef.setInterval = typeof setInterval !== 'undefined' ? setInterval.bind(global) : windowRef.setInterval; } catch (_) {}
    try { windowRef.clearTimeout = typeof clearTimeout !== 'undefined' ? clearTimeout.bind(global) : windowRef.clearTimeout; } catch (_) {}
    try { windowRef.clearInterval = typeof clearInterval !== 'undefined' ? clearInterval.bind(global) : windowRef.clearInterval; } catch (_) {}
    try { windowRef.fetch = typeof fetch !== 'undefined' ? fetch.bind(global) : windowRef.fetch; } catch (_) {}
    ...
    windowRef.pluginApi = {
      emit(eventType, payload) {
        try {
          parentPort.postMessage({ type: 'plugin_event', eventType, payload, pluginId });
        } catch (_) {}
      }
    };
    ...
    try {
      ...
      console.error('[Worker] 读取 manifest.json 失败:', e && e.message ? e.message : String(e));
    }

    const bundledBase = path.join(process.cwd(), 'buildResources', 'plugins', String(pluginId || ''));
    const resolveFile = (rel) => {
      const p1 = path.join(pluginDir, rel);
      const p2 = path.join(bundledBase, rel);
      try { if (testMode && fs.existsSync(p2)) return p2; } catch (_) {}
      return p1;
    };
    ...
    for (const libRel of allLibs) {
      try {
        ...
        if (exportsObj && (typeof exportsObj === 'function' || (typeof exportsObj === 'object' && Object.keys(exportsObj).length > 0))) {
          ...
          console.info('[Worker] lib loaded:', { libRel, basename, t: typeof exportsObj, k: exportsObj && Object.keys(exportsObj) });
        } else {
          console.warn('[Worker] lib export invalid:', { libRel });
        }
      } catch (e) {
        const msg = e && e.message ? e.message : String(e);
        console.error('[Worker] 加载 lib 失败:', msg);
        throw e;
      }
    }
    ...
    const mainPath = (() => {
      ...
      return path.resolve(pluginPath || (workerData && workerData.pluginPath) || '');
    })();

    const indexContent = fs.readFileSync(mainPath, 'utf-8');
    runModuleInWindow({ code: indexContent, filename: mainRel || path.basename(mainPath), windowRef, requireFn: pluginRequire });

    if (typeof windowRef.afterLoaded === 'function') {
      try { await windowRef.afterLoaded(); } catch (e) { console.error('[Worker] afterLoaded 执行失败:', e && e.message ? e.message : String(e)); }
    } else if (typeof windowRef.afterloaded === 'function') {
      try { await windowRef.afterloaded(); } catch (e) { console.error('[Worker] afterloaded 执行失败:', e && e.message ? e.message : String(e)); }
    }

    pluginLoaded = true;
    console.info('[Worker] plugin loaded', { pluginPath, mode: 'happydom-modules' });
  } catch (error) {
    const msg = error && error.message ? error.message : String(error);
    console.error('[Worker] loadPlugin error:', msg);
...
function startMemoryMonitor() {
  ...
  memoryInterval = setInterval(() => {
    try { const usage = process.memoryUsage().heapUsed; parentPort.postMessage({ type: 'memory_usage', usage }); } catch (_) {}
  }, 10000);
}

function stopMemoryMonitor() { if (memoryInterval) { clearInterval(memoryInterval); memoryInterval = null; } }
...
parentPort.on('message', async (message) => {
  try {
    ...
    if (message && message.type === 'execute') {
      ...
      console.info('[Worker] execute', { method, argsLen: args.length });
      if (!pluginLoaded) { await loadPlugin(workerData.pluginPath); startMemoryMonitor(); }
      let targetFn = null;
      ...
      if (!targetFn) {
        if (!optional) { console.warn('[Worker] method not found', { method }); parentPort.postMessage({ type: 'result', error: `Method ${method} not found on window` }); }
      ...
      try {
        ...
        parentPort.postMessage({ type: 'execution_complete' });
        console.info('[Worker] complete', { method });
      }
      return;
    }
  } catch (err) {
...
process.on('exit', async () => {
  try { stopMemoryMonitor(); if (windowRef && typeof windowRef.cleanup === 'function') { await windowRef.cleanup(); } } catch (_) {}
});
